...
You need a fylr licence including Authentication to use LDAP. To enable fylr users to **log in** with LDAP accounts, scroll the **User Management** page to LDAP, _above_ SAML:
...
* in **USER MAPPING**, the **Target**: **Email** is set to **Value** `%(mail)s`\
  \
...
<figure><img src="../.gitbook/assets/image (1) (1) (1) (1).png" alt=""><figcaption><p>tested example for ldap.forumsys.com</p></figcaption></figure>
...
*   In the settings for a fylr group, enter a string that matches one LDAP group. For the example of ldap.forumsys.com, we created the fylr group `scientists` and since we chose `cn` above in **Group Mapping**, we now have to use the string `Scientists`, as this is the value in that group's common name (`cn`). In the fylr frontend this is done here:
    ...
    <figure><img src="../.gitbook/assets/image (2) (1) (1) (1).png" alt=""><figcaption><p>fylr frontend > Rights Management > Groups > Choose group > Authentication Services > LDAP section > Add row</p></figcaption></figure>
