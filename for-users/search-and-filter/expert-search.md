...
Use the **filter input field** on the top of the expert search to find specific fields and reduce the expert search form. By default, nested fields only show the fields that match. Enable "Show Complete Nested Field(s)" to show all other fields in the matching nested.
