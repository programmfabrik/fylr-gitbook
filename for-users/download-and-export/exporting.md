...
{% hint style="info" %}
Please note that the **Deep Link User** must have the required permissions to access the referenced content. If the user lacks the necessary permissions, the deep link cannot be resolved and the server will return an **HTTP 403 (Forbidden)** response.
{% endhint %}
