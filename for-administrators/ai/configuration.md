...
The search assistant above takes a chain, and so does [tagging](tagging.md) — the embedding model does not: vectors of two different models cannot be compared, so a fallback would silently poison the index.
