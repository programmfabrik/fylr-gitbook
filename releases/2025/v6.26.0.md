...
* This **fylr** version officially supports the new [PostgreSQL
  18](https://www.postgresql.org/about/news/postgresql-18-released-3142/). We
  recommend this version for new installations. [36ae2790a]
...
* **/inspect/files**: Action `map_to_local_storage` now sets the
  ...
  the files better. The action can still be repeated as `leave_on_remote` is not
  a required flag for the action file filter. Also, always show status message
  in file list. And last but not least, test reachability of remote url for file
  with leave on remote turned on with action `resync`.[899e6940f] [b35469f4a]
  [77b036023]
...
* **Filename replacements**: Add `_standard` and more column types. This adds
  ...
  _number_, _double_, _date_, _datetime_, _daterange_. In additiona, **linked
...
* **/api/pool|objecttype**: Added a per filed configuration for captions. Also,
  introduced a 202 process to resync files. If watermark or caption
  configuration changes, the API responses with a 202 when pools or objecttypes
  a saved. This includes more replacements too (see _Filename replacements_)
...
* **Cookbook `_metadata`**: Introduce service `ocr`. This service is used by
  ...
  [532498eb7]
* **/api/db**: Improved checks for deferred checks. he checks for errors
  `DeferredLinkedObjectMatchingSystemObjectId` and
  `DeferredLinkedObjectWrongObjecttype` are now only performed for newly
  inserted objects (version 1), which saves time. This patch also make the check
...
* **/api/config**: New 202 process to **ignore errors**. If the base config
  ...
  the errors. aseConfigMap can be skipped for saving if their attribute Savable
...
* **/api/schema**: Remove deletd object types from transitions. Before loading
...
* **fylr backup**: Fixed panic when parsing B.C. dates. Parsing BC dates
  required loca to be loaded. During backup we don't have that, added a
  fallback. [bb70270da]
