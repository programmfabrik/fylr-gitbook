...
* The **fylr** binary now embeds the webfrontend code. It is no longer necessary nor advised to reference `webapp: path:` in your `fylr.yml`. It should be removed, unless you are a developer and working with fast changing code for the webfrontend.
...
* **/api/search**: Support new subfield `.metdata_fulltext` for type `file`. This can be used from inside the query language too.
...
* **/api/search**: **Filtering by the metadata of a file** (like the PDF text) is now possible using the new subfield `.metdata_fulltext`. Works in query language too.
...
* **Allow size configuration up to 1000px** for images and videos (before the limit was 2000px).
