...
* A new subcomment `fylr convert` is used to simplify our recipes and be able to make file preview renderings more powerful and better in a future release. In order for this to work, you must add a block to you `fylr.yml`. If you use the `fylr.default.yml` as basis (and that we highly recommend), you don't need to change anything. You **must** add a command **fylr** (see below).
